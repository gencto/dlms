...
class DlmsSimulator {
  ...
  void _handleConnection(Socket socket) {
    ...
    _serverChallenge = Uint8List.fromList([
      0x01,
      0x02,
      0x03,
      0x04,
  ...
  void _handleGetRequest(
    ...
    if (!_authenticated) {
      _send(
        socket,
        Uint8List.fromList([0xC4, 0x01, 0x00, 0x01, 0x01]),
        clientAddr,
        serverAddr,
      );
      return;
    ...
    // Router
    if (classId == 8 && attrId == 2) {
      // Clock Time
      result = _createDateTimeValue();
    } else if (classId == 3 && attrId == 2) {
      // Register Value
      if (obis == ObisCode(1, 0, 1, 8, 0, 255)) {
        // Active Energy
        _energyCounter++;
        result = DlmsValue(_energyCounter, 6); // Uint32
      }
    } else if (classId == 70 && attrId == 2) {
      // Disconnect State
      result = DlmsValue(!_breakerOpen, 3); // Boolean (True=Closed/Connected)
    } else if (classId == 7 &&
        attrId == 2 &&
        obis == ObisCode(1, 0, 99, 1, 0, 255)) {
    ...
    if (result != null) {
      ...
      _send(
        socket,
        Uint8List.fromList([0xC4, 0x01, invokeId, 0x01, 0x01]),
        clientAddr,
        serverAddr,
      );
    }
  ...
  void _handleSetRequest(
    ...
    _send(
      socket,
      Uint8List.fromList([0xC5, 0x01, data[2], 0x00]),
      clientAddr,
      serverAddr,
    );
  }
  ...
  void _handleActionRequest(
    ...
    _send(
      socket,
      Uint8List.fromList([0xC7, 0x01, invokeId, 0x00]),
      clientAddr,
      serverAddr,
    );
  }

  void _sendGetResponse(
    Socket socket,
    int invokeId,
    DlmsValue value,
    int clientAddr,
  ...
  Uint8List _generateFullProfile() {
    ...
    for (int i = 0; i < 50; i++) {
      rows.add(DlmsValue([_createDateTimeValue(), DlmsValue(i * 10, 6)], 2));
    }
