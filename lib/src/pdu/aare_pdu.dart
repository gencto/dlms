...
class AarePdu {
  ...
  void _parse() {
    ...
    if (endOffset > rawData.length) {
       ...
       throw FormatException('AARE Length $outerLen exceeds buffer size ${rawData.length}');
    }
      ...
      if (tag == 0xA2) {
        ...
        if (len > 2 && rawData[valueOffset] == 0xA0) {
           ...
           if (rawData[innerOff] == 0x04) {
             ...
             authenticationValue = rawData.sublist(innerOff, innerOff + octetLen);
           }
        } else {
           // Maybe direct GraphicString (80)?
           if (rawData[valueOffset] == 0x80) {
              authenticationValue = rawData.sublist(valueOffset + 2, valueOffset + len);
           } else {
        ...
        // User Information
        if (len > 0) {
            _parseUserInformation(rawData.sublist(valueOffset, valueOffset + len));
        }
      }
  ...
  void _parseUserInformation(Uint8List info) {
    ...
    if (info[0] == 0x04) {
      ...
      final xdlmsBytes = info.sublist(offset + headerLen, offset + headerLen + len);
  ...
  void _parseXdlmsInitiateResponse(Uint8List data) {
    ...
    if (tag != 0x08) { // InitiateResponse tag
      // print('Not an InitiateResponse: $tag');
      return;
    }
    
    reader.readUint8(); // Negotiated Quality of Service (Optional/Default) - usually skipped or read if present? 
